...
function annotatePublishedLesson(lesson) {
  return {
    ...
    lessonRuntimeSourceLabel:
      lesson.lessonRuntimeSourceLabel || LESSON_RUNTIME_SOURCE_LABELS.PUBLISHED,
  };
}
...
function normalizeDraftLesson(lesson) {
  ...
  if (isPlayableDraftRuntimeSource(lesson.lessonRuntimeSource)) {
    return {
      ...lesson,
      lessonRuntimeSourceLabel:
        lesson.lessonRuntimeSourceLabel ||
  ...
  return {
    ...
    lessonRuntimeSource: LESSON_RUNTIME_SOURCE.PLAYABLE_DRAFT,
    lessonRuntimeSourceLabel:
      lesson.lessonRuntimeSourceLabel || LESSON_RUNTIME_SOURCE_LABELS.PLAYABLE_DRAFT,
  };
}
...
function buildLessonsForSelection(registeredLessons, lesson) {
  ...
  const existingLessonIndex = registeredLessons.findIndex(
    (registeredLesson) => registeredLesson.lessonId === lesson.lessonId
  );

  if (existingLessonIndex >= 0) {
    return registeredLessons.map((registeredLesson, lessonIndex) =>
      lessonIndex === existingLessonIndex
        ? {
            ...registeredLesson,
...
export async function selectLessonFromLocation({
  ...
  const { registeredLessons, findLesson, getDefaultLessonId } =
  ...
  return buildLessonSelection(registeredLessons, await defaultLesson.loadLesson(), null);
}
