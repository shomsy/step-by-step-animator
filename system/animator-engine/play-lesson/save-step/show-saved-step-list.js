...
export function showSavedStepList({ lessonParts, savedStepNumbers, currentStepNumber, steps }) {
  const savedItems = savedStepNumbers
    .map((stepNumber) => ({ stepNumber, step: steps[stepNumber] }))
    .filter(({ step }) => Boolean(step));

  lessonParts.saveStepButton.classList.toggle(
    'active',
    savedStepNumbers.includes(currentStepNumber)
  );
  ...
  lessonParts.savedStepList.innerHTML = savedItems
    .map(
      ({ stepNumber, step }) => `
