...
function validateSceneCrossReferences({ sceneSteps, artifactIds, theoryAnchors, theoryEnabled }) {
  const hasTheoryScenes = sceneSteps.some((step) =>
    (step.scenes || []).some((scene) => Boolean(scene.theory))
  );
  ...
  sceneSteps.forEach((step) => {
    (step.scenes || []).forEach((scene) => {
      if (scene.focus && !artifactIds.has(scene.focus.artifactId)) {
        throw new Error(
          `Scene "${scene.sceneId}" in step "${step.stepId}" references unknown artifact "${scene.focus.artifactId}".`
        );
      }

      if (scene.code && !artifactIds.has(scene.code.activeArtifactId)) {
        throw new Error(
          `Scene "${scene.sceneId}" in step "${step.stepId}" references unknown active artifact "${scene.code.activeArtifactId}".`
        );
      }
      ...
      if (scene.theory) {
        if (!theoryAnchors || !theoryAnchors.size) {
          throw new Error(
            `Scene "${scene.sceneId}" in step "${step.stepId}" references theory.anchor "${scene.theory.anchor}" but theory.md was not provided.`
          );
        }

        if (!theoryAnchors.has(scene.theory.anchor)) {
          throw new Error(
            `Scene "${scene.sceneId}" in step "${step.stepId}" references unknown theory.anchor "${scene.theory.anchor}".`
          );
        }
      }
    });
...
export function normalizeLessonMeta(attributes, body, goalImageSrcOverride) {
  ...
  return {
    ...
    goal: {
      ...
      imageCaption:
        normalizeString(goal.imageCaption) || normalizeString(attributes.goalImageCaption),
    },
    homework: {
      enabled: Boolean(homework.enabled ?? Array.isArray(attributes.homeworkItems)),
      title:
        normalizeString(homework.title) || normalizeString(attributes.homeworkTitle) || 'Homework',
      items: Array.isArray(homework.items)
        ? homework.items.map((item) => normalizeString(item)).filter(Boolean)
        : [],
    },
    ...
    tags: Array.isArray(attributes.tags)
      ? attributes.tags.map((tag) => normalizeString(tag)).filter(Boolean)
      : [],
...
export function buildCompiledLesson({
  ...
  const teachingSteps = sceneSteps.map((step) =>
    createRuntimeStep(step, lessonMeta.preview.type, deriveFocusArtifactId(step))
  );
  ...
  return {
    ...
    lessonIntro: lessonMeta.lessonIntro,
    lessonIntroHtml:
      renderMarkdown(lessonBody) ||
      (lessonMeta.lessonIntro ? `<p>${escapeHtml(lessonMeta.lessonIntro)}</p>` : ''),
