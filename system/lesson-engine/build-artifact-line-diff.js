...
function readCommonSuffixLength(previousLines, currentLines, prefixLength) {
  ...
  while (
    ...
    && previousLines[previousLines.length - 1 - suffixLength] === currentLines[currentLines.length - 1 - suffixLength]
  ) {
    suffixLength += 1;
  }
...
function buildUnchangedEntries(lines, previousLineOffset, currentLineOffset) {
  return lines.map((lineText, index) => createDiffEntry(
    'unchanged',
    lineText,
...
function buildFallbackChangedEntries(previousLines, currentLines, previousLineOffset, currentLineOffset) {
  const removedEntries = previousLines.map((lineText, index) => createDiffEntry(
    'removed',
    lineText,
    previousLineOffset + index + 1,
    null
  ));
  const addedEntries = currentLines.map((lineText, index) => createDiffEntry(
    'added',
    lineText,
    null,
    currentLineOffset + index + 1
...
function buildExactMiddleEntries(previousLines, currentLines, previousLineOffset, currentLineOffset) {
...
function buildMiddleEntries(previousLines, currentLines, previousLineOffset, currentLineOffset) {
  ...
  if (!previousLines.length || !currentLines.length) {
    return buildFallbackChangedEntries(previousLines, currentLines, previousLineOffset, currentLineOffset);
  }
  ...
  return buildExactMiddleEntries(previousLines, currentLines, previousLineOffset, currentLineOffset);
}
...
export function buildArtifactLineDiff(previousLines, currentLines) {
  ...
  const suffixLength = readCommonSuffixLength(normalizedPreviousLines, normalizedCurrentLines, prefixLength);
