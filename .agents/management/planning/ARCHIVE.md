...
- 2026-03-30 23:35 CEST - PLAN-012 completed: repackaged lesson documentation around one Serbian canonical author-facing contract, removed the competing repo-local lesson authoring guide, and tightened the shipped lesson validation seam so smoke coverage no longer depends on child-process stdout behavior.
- 2026-03-30 04:14 CEST - PLAN-011 completed: added one shared step-level artifact diff truth so playback and Write Mode both show git-style added and removed lines for the active artifact, then proved the contract with browser and contract coverage.
