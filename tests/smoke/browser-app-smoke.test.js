...
function readFrontmatterValue(sourceMarkdown, fieldName) {
  const match = String(sourceMarkdown || '').match(new RegExp(`^${fieldName}:\\s*(.+)$`, 'm'));
  return match?.[1]?.trim() || '';
}
...
test(
  'browser app smoke covers boot, navigation, lesson switch, script-lesson loading, and preview isolation',
  { timeout: 45000 },
  async () => {
    ...
    const shippedLessonSource = await fs.readFile(shippedLessonFilePath, 'utf8');
    const shippedLessonTitle = readFrontmatterValue(shippedLessonSource, 'lessonTitle');
    ...
    try {
      ...
      await waitForCondition(
        async () =>
          page.evaluate(() => {
            ...
            return (
              lessonPicker?.value === '01-build-sidebar' &&
      ...
      const initialHeading = await page.$eval(
        '#lessonHeading',
      ...
      assert.ok(
        await page.$eval(
          '#livePreviewFrame',
          (frame) => (frame.getAttribute('srcdoc') || '').length > 0
        )
      );
      ...
      assert.deepEqual(
        await page.$eval('#stepSpeechBrowserVoiceSelect', (element) =>
      ...
      assert.equal(await page.$eval('#pauseStepSpeechBtn', (element) => element.disabled), false);
      ...
      await waitForCondition(
        async () =>
          page.$eval('#speakStepBtn span', (element) => element.textContent === 'Nastavi'),
      ...
      await waitForCondition(
        async () =>
          page.evaluate(() => window.__speechLog.some((entry) => entry.type === 'resume')),
      ...
      const spokenEntries = await page.evaluate(() =>
        window.__speechLog.filter((entry) => entry.type === 'speak')
      );
      ...
      await waitForCondition(
        async () =>
          page.evaluate(
            () => document.getElementById('lessonPicker')?.value === '02-build-top-navigation'
          ),
        'the switched lesson to load'
      );
      const headingAfterLessonSwitch = await page.$eval(
        '#lessonHeading',
        (element) => element.textContent?.trim() || ''
      );
      ...
      await waitForCondition(
        async () =>
          page.evaluate(
            () => document.getElementById('lessonPicker')?.value === '09-human-first-script-demo'
          ),
        'the script-authored lesson to load'
      );
      const headingAfterScriptLessonSwitch = await page.$eval(
        '#lessonHeading',
        (element) => element.textContent?.trim() || ''
      );

      assert.match(page.url(), /[?&]lesson=09-human-first-script-demo\b/);
      assert.equal(headingAfterScriptLessonSwitch, shippedLessonTitle);
      assert.equal(
        await page.$eval(
          '#stepNumber',
          (element) => element.textContent?.startsWith('Prizor 1 /') || false
        ),
        true
      );
      ...
      await Promise.all([
        page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
        page.click('#authoringBtn'),
      ]);

      await waitForCondition(
        async () => new URL(page.url()).pathname === '/content/lesson/09-human-first-script-demo',
        'the player authoring handoff to use the clean lesson editor route'
      );
      await waitForCondition(
        async () => page.$('#authoringScriptEditor').then(Boolean),
        'the authoring workspace after the player handoff'
      );
