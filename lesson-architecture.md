...
- `when`: kada se ovaj blok primenjuje (step boundary identifikator)
- `into`: gde se ubacuje u finalnom kodu (slot marker ime)
- `lines`: šta se ubacuje (niz linija koda)
...
- `when`: jasno kaže "u kom koraku se ovo dešava" (flow-first)
- `into`: direktno govori "u koju poziciju se ubacuje" (DSL-friendly)
- `lines`: intuitivno "šta je sadržaj" (developer-friendly)

### 4.2 Prednosti intuitivnih ključeva

- Čita se kao engleski: "when X, insert lines into Y"
- Lakše za autore lekcija da razumeju bez dokumentacije
- Manje grešaka u pisanju (when vs step, into vs slot, lines vs code)
